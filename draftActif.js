...
(function () {
     ...
     *
     * Le bandeau disait « Les Castors choisit » sans jamais dire qui l'on
     * était : dans un pool de huit, on ne savait pas toujours si c'était soi.
     */
    function ecrireMonEquipe(cible, prefixe) {
        if (!cible) return;
        const me = typeof getUserTeam === "function" ? getUserTeam() : null;
        cible.replaceChildren();
        if (!me) return;
        const puce = document.createElement("span");
        puce.className = "fz-me-chip";
        puce.textContent = `${prefixe} · ${me}`;
        cible.appendChild(puce);
    }
...
function _isCategoryFull(positionCode) {
    try {
        ...
        // Une place de banc libre accueille n'importe quel joueur (jamais un
        // club) : la catégorie n'est alors pas « pleine » pour le choix.
        const banc = typeof window.fzQuotaBanc === "function" ? window.fzQuotaBanc(draftData) : 0;
        if (positionCode !== "T" && (t.bench || []).length < banc) return false;
...
(function () {
    ...
    /* ---- Fenêtre « Connexion perdue » ----
       La pastille de connexion est masquée sous 1100px, et même sur bureau
       elle se remarque mal pendant qu'on fait défiler la liste. Une coupure
       qui dure fait donc apparaître une fenêtre par-dessus la salle : on ne
       choisit pas un joueur dans le vide. Son retour est annoncé par une
       coche, puis la salle se resynchronise d'elle-même. */
    const DELAI_FENETRE_MS = 1500;   // une microcoupure ne mérite pas d'écran
    const DUREE_RETOUR_MS = 1400;
    let minuteurFenetre = null;
    let minuteurRetour = null;
    let fenetreMontree = false;

    function fenetre() {
        let el = document.getElementById("fzConnOverlay");
        if (el) return el;
        document.body.insertAdjacentHTML("beforeend", `
            <div class="fz-conn-overlay" id="fzConnOverlay" role="alertdialog" aria-modal="true"
                 aria-labelledby="fzConnTitle" aria-describedby="fzConnText" hidden>
                <div class="fz-conn-box">
                    <div class="fz-conn-icon" aria-hidden="true">
                        <span class="fz-conn-spinner"></span>
                        <span class="fz-conn-check">✓</span>
                    </div>
                    <h2 class="fz-conn-title" id="fzConnTitle"></h2>
                    <p class="fz-conn-text" id="fzConnText"></p>
                    <button type="button" class="fz-conn-retry" id="fzConnRetry">Réessayer maintenant</button>
                </div>
            </div>`);
        el = document.getElementById("fzConnOverlay");
        document.getElementById("fzConnRetry").addEventListener("click", reessayer);
        return el;
    }

    function reessayer() {
        try { if (typeof socket !== "undefined" && socket && !socket.connected) socket.connect(); } catch (e) {}
        if (typeof loadDraftData === "function") loadDraftData();
    }

    function textesFenetre() {
        const el = fenetre();
        const hors = etat === "offline";
        el.querySelector("#fzConnTitle").textContent = hors ? "Vous êtes hors ligne" : "Connexion perdue";
        el.querySelector("#fzConnText").textContent = hors
            ? "Vérifiez votre connexion Internet. Le repêchage vous attend : rien n’est choisi à votre place."
            : "Reconnexion en cours… Vos choix sont en pause le temps de retrouver la salle.";
    }
    ...
    function cacherFenetre() {
        const el = document.getElementById("fzConnOverlay");
        if (!el) return;
        el.classList.remove("is-open");
        setTimeout(function () { if (!el.classList.contains("is-open")) el.hidden = true; }, 220);
        fenetreMontree = false;
    }

    function rendreFenetre() {
        clearTimeout(minuteurFenetre);
        if (etat !== "ok") {
            if (fenetreMontree) { textesFenetre(); return; }
            minuteurFenetre = setTimeout(montrerFenetre, DELAI_FENETRE_MS);
            return;
        }
        if (!fenetreMontree) return;
        // Retour : la coche, le mot, puis la salle reprend — et relit l'état
        // du repêchage, qui a pu avancer pendant la coupure.
        const el = fenetre();
        el.classList.add("is-back");
        el.querySelector("#fzConnTitle").textContent = "Connexion rétablie";
        el.querySelector("#fzConnText").textContent = "Vous êtes de retour dans la salle.";
        if (typeof loadDraftData === "function") loadDraftData();
        minuteurRetour = setTimeout(cacherFenetre, DUREE_RETOUR_MS);
    }
