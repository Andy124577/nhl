...
const poolOps = require('./poolOps.js');
...
/** Préfixe des pools ouverts avant le changement de nom : toujours reconnus. */
...
function creerPool(username, options = {}) {
    ...
    // Une personne, une équipe, qui porte son nom : la file ne demande rien,
    // et chacun pourra la renommer ensuite comme dans n'importe quel pool.
    const teams = { [poolOps.nomEquipeLibre({}, username)]: poolOps.equipeVide([username]) };
    ...
    return {
        ...
        allowTrades: true,
        instant: true,
        // Écrits à la création, lus par compatible() : deux personnes venues
        // pour deux formats ou deux saisons ne doivent pas finir dans le même
        // salon.
        formatVersion: options.formatVersion || VERSION_FORMAT,
...
function inscrire(pool, username) {
    ...
    const resultat = poolOps.inscrireParticipant(pool, {
        username,
        teamName: poolOps.nomEquipeLibre(pool, username)
    });
    return resultat.ok ? resultat.teamName : null;
}
...
function retirer(pool, username) {
    ...
    // Son équipe n'existait que par elle : elle part avec, et libère la place.
    if (poolOps.equipeInutilisee(equipe)) delete pool.teams[nom];
    return nom;
}
...
/* ────────────────────────────────────────────────────────────────────────
 * FORMAT, ÉTAT, COMPATIBILITÉ
 *
 * Trois notions que la file d'attente n'avait pas, et dont l'absence se
 * payait à chaque cas limite.
 *
 * Le FORMAT dit ce qu'est un repêchage instantané : quatre personnes, une
 * équipe chacune, cumulatif, mêmes quotas. Sa version est écrite sur le pool.
 * Changer le format demain ne doit pas mélanger dans un même salon des gens
 * venus pour deux formats différents — c'est exactement ce qui arriverait si
 * la compatibilité se résumait à « pool instantané qui n'a pas démarré ».
 *
 * L'ÉTAT nomme les quatre moments : en attente, démarrage imminent, en
 * repêchage, terminé. Ils étaient déduits ailleurs, à chaque fois un peu
 * différemment — un salon complet mais pas encore parti n'avait pas de nom du
 * tout, et le compte à rebours n'avait donc rien à décrire.
 * ──────────────────────────────────────────────────────────────────────── */

/** Version du format. À incrémenter quand la forme d'un salon change. */
 ...
 * L'état d'un salon.
 *
 * `demarrageDemande` vient de la présence, pas de la base : le compte à
 * rebours vit en mémoire du serveur et n'a pas à être persisté pour être
 * décrit. Ce qui est persisté, c'est son résultat — l'ordre de sélection.
 */
function etatPool(pool, { demarrageDemande = false } = {}) {
    if (!pool) return null;
    if (repechageCommence(pool)) {
        return checkIfDraftComplete(pool) ? ETATS.TERMINE : ETATS.REPECHAGE;
    }
    if (demarrageDemande) return ETATS.DEMARRAGE;
    return ETATS.ATTENTE;
}

/**
 * Ce salon peut-il accueillir quelqu'un qui arrive maintenant ?
 *
 * Quatre conditions, et elles se vérifient ensemble : même version de format,
 * même saison, une place libre, et pas encore parti. La saison est ce qui
 * empêche un salon oublié depuis le printemps de servir de point de rendez-vous
 * en octobre — ses participants ne joueraient pas la même année.
 */
function compatible(nom, pool, { versionFormat = VERSION_FORMAT, saison = null } = {}) {
    if (!estPoolInstantane(nom, pool)) return false;
    if ((pool.formatVersion || VERSION_FORMAT) !== versionFormat) return false;
    if (saison && pool.season && pool.season !== saison) return false;
    if (repechageCommence(pool)) return false;
    return placesRestantes(pool) > 0;
}

/**
 * Le salon qui doit accueillir le prochain arrivant, à format et saison
 * donnés — la version de poolEnAttente() qui tient compte du format.
 */
function salonCompatible(draftData, options = {}) {
    const candidats = Object.entries(draftData || {})
        .filter(([nom, pool]) => compatible(nom, pool, options));
...
/**
 * Transfère le rôle de créateur quand la personne qui l'occupe s'en va.
 *
 * Le créateur d'un salon instantané n'a rien fondé — il est simplement arrivé
 * le premier — mais c'est lui qui pourra débloquer la salle en sautant un tour
 * qui traîne. S'il part avant le départ sans transmettre, les trois autres
 * se retrouvent avec un administrateur injoignable.
 *
 * Renvoie le nouveau créateur, ou null si le salon est vide.
 */
function transfererCreateur(pool, partant) {
    if (!pool || pool.creator !== partant) return pool ? pool.creator || null : null;
    const restants = membres(pool).filter(m => m !== partant);
    pool.creator = restants.length > 0 ? restants[0] : null;
    return pool.creator;
}

/**
 * Ce que le salon d'attente affiche : qui est là, combien il en manque,
 * depuis quand on attend.
 *
 * Aucune estimation de délai : personne ne peut savoir quand trois inconnus
 * vont cliquer. Le temps écoulé est un fait, une prévision n'en serait pas un.
 */
function vueSalon(nom, pool, { presence = [], demarrageDemande = false, maintenant = Date.now() } = {}) {
    if (!pool) return null;
    const inscrits = membres(pool);
    const max = pool.maxPlayers || JOUEURS_PAR_POOL;
    const ouvertDepuis = Date.parse(pool.createdAt || '') || null;

    return {
        poolName: nom,
        state: etatPool(pool, { demarrageDemande }),
        formatVersion: pool.formatVersion || VERSION_FORMAT,
        participants: inscrits.length,
        maxPlayers: max,
        placesRestantes: Math.max(0, max - inscrits.length),
        membres: inscrits,
        presence,
        creator: pool.creator || null,
        attenteMs: ouvertDepuis ? Math.max(0, maintenant - ouvertDepuis) : null,
        totalSelections: totalSelections(pool) * Math.max(1, equipesEligibles(pool).length)
    };
}
