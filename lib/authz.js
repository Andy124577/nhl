 ...
 * de passe et sa copie chiffrée, plus la révision dont il provient.
 *
 * La copie chiffrée (`passwordSecret`, lib/poolSecret.js) ne sort que par sa
 * propre route, réservée à la personne qui a créé le pool : cette vue-ci est
 * diffusée à TOUS les membres, en HTTP comme sur le canal temps réel.
 *
...
function vueMembre(nom, pool, revision = null) {
    if (!pool || typeof pool !== 'object') return null;
    const { passwordHash, passwordSecret, ...reste } = pool;
    return {
        ...reste,
        name: nom,
        isMember: true,
