...
/**
 * Le lundi où s'ouvre le calendrier d'un pool tête-à-tête.
 *
 * Le plus TARDIF entre le lundi d'aujourd'hui et celui du premier match de la
 * saison régulière. Les deux bornes comptent :
 *
 *   - le calendrier se bâtissait sur le lundi du REPÊCHAGE. Un pool tiré en
 *     septembre ouvrait donc sa semaine 1 sur une semaine sans un seul match
 *     de saison régulière : le duel s'affichait « en cours » à 0,0 contre 0,0,
 *     puis se finalisait sur une feuille vide ;
 *   - un pool tiré en janvier ne doit pas pour autant faire commencer sa
 *     saison en octobre : ses semaines 1 à 13 seraient déjà échues, et le
 *     rattrapage les fermerait toutes à zéro au premier passage.
 *
 * Sans date d'ouverture connue — calendrier de la LNH injoignable — on s'en
 * tient au lundi courant : c'est le comportement d'avant, jamais pire.
 */
function lundiDepartSaison(fenetreSaison, aujourdhui = dates.journeeLocale()) {
    const lundiCourant = dates.lundiDe(dates.journeeDe(aujourdhui));
    const ouverture = fenetreSaison && fenetreSaison.regularSeasonStartDate;
    const lundiOuverture = ouverture ? dates.lundiDe(dates.journeeDe(ouverture)) : null;
    if (!lundiOuverture) return lundiCourant;
    if (!lundiCourant) return lundiOuverture;
    return lundiOuverture > lundiCourant ? lundiOuverture : lundiCourant;
}
