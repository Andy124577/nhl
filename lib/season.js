...
/** Saison précédente : 20262027 → 20252026. */
function previousSeasonId(seasonId) {
    const opening = Number(String(seasonId).slice(0, 4)) - 1;
    return Number(`${opening}${opening + 1}`);
}

/**
 * Saison dont les statistiques de joueur doivent être affichées.
 *
 * currentSeasonId() bascule le 1er juillet, mais la saison régulière ne
 * commence qu'en octobre : entre les deux, elle désigne une saison dont
 * aucun match n'a été joué. Les pages qui affichent des statistiques
 * demandaient donc à la LNH les totaux de 20262027 dès juillet 2026, n'en
 * recevaient rien, et remplissaient tout le tableau de zéros — le tri par
 * points n'avait alors plus rien à trier.
 *
 * La règle, unique et partagée : tant que la saison régulière n'a pas
 * commencé, la référence reste la dernière saison complétée.
 *
 *   septembre 2026 (rien n'a commencé)  → 20252026
 *   octobre 2026   (la saison a débuté) → 20262027
 *
 * À ne pas confondre avec currentSeasonId(), qui reste la bonne réponse
 * pour tout ce qui concerne la saison en cours elle-même : le pointage
 * d'un pool vaut zéro avant le premier match, il ne vaut pas les points
 * de l'an dernier (voir seasonHasStarted).
 */
function statsSeasonId(window, now = new Date()) {
    const encours = (window && window.seasonId) ? window.seasonId : seasonIdForDate(now);
    return seasonHasStarted(window, now) ? encours : previousSeasonId(encours);
}

/** Même règle, en chaîne — pour les colonnes SQL et les URL de l'API. */
function statsSeasonString(window, now = new Date()) {
    return String(statsSeasonId(window, now));
}

/**
 * La même règle, mais synchrone, d'après le dernier calendrier lu.
 *
 * Pour les endroits qui ne peuvent pas attendre : une URL de photo au milieu
 * d'un gabarit, par exemple. Tant qu'aucun getSeasonWindow() n'a eu lieu,
 * elle répond la saison courante — le comportement d'avant, donc jamais pire.
 * Le serveur en résout une au démarrage et à chaque /current-stats, si bien
 * qu'en pratique le cache est chaud.
 */
function cachedStatsSeasonString(now = new Date()) {
    return String(statsSeasonId(windowCache ? windowCache.window : null, now));
}

/**
 * La règle ci-dessus, résolue d'un seul appel : calendrier compris.
 * C'est ce que le serveur consomme, pour n'avoir qu'un endroit à appeler.
 */
async function getStatsSeason({ now = new Date(), fetchImpl, force = false } = {}) {
    const window = await getSeasonWindow({ now, fetchImpl, force });
    const seasonId = statsSeasonId(window, now);
    return {
        seasonId,
        season: String(seasonId),
        label: seasonLabel(seasonId),
        hasStarted: seasonHasStarted(window, now),
        window
    };
}
