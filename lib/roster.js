...
/**
 * Borne le journal des transactions : les `keep` plus récentes de CHAQUE
 * type, dans l'ordre reçu (du plus récent au plus ancien).
 *
 * Une borne unique pour tout le journal laissait un type en chasser un
 * autre. Au camp d'entraînement, chaque retranchement est un départ : fin
 * septembre 2026, 216 départs en dix jours avaient repoussé hors de la
 * fenêtre les 75 échanges de l'été, et l'onglet « Échanges » de l'accueil
 * restait vide — alors que l'accueil n'affiche même pas les départs.
 */
function trimTransactionLog(transactions, keep) {
    const vus = new Map();
    return (transactions || []).filter(t => {
        const n = (vus.get(t.type) || 0) + 1;
        vus.set(t.type, n);
        return n <= keep;
    });
}
